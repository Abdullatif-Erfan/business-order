...
function showNextRow(id)
{
    ...
    $('#pre_list_id'+id).prop('required', true);
...
function removeCurrentRow(id,deleteId = 0)
{
    var prevId = id - 1;
    var elem = document.getElementById("row" + id);
    ...
    $('#pre_list_id'+id).removeAttr('required');
    ...
    $('#cur_total'+id).removeAttr('required');
    $('#total'+id).removeAttr('required');
    $('#commission'+id).removeAttr('required');
    $('#total_commission'+id).removeAttr('required');
    $('#total_transport'+id).removeAttr('required');
    ...
    if(parseInt(deleteId) > 0)
    {
        $.ajax({
            type: 'POST',
            data: { deleteId: deleteId, table: 'bought_item_details' },
            url: "<?php echo base_url() . 'buy/buying/deletThisItemDetails'; ?>",
				  ...
				  <div class="card" style="min-height: 400px"> 
                    <div class="card-header" style="padding: 10px;">
                     ...
                     <?php
                        ...
                        echo form_open('itemBuyingUpdate',$attributes);  
                        ?>
                         ...
                         <input type="hidden" name="id" value="<?=$med_bought[0]['id']?>" >
                         <input type="hidden" name="code" value="<?= show_journal_code_by_times($med_bought[0]['times'])?>"/>
                         <input type="hidden" name="times" value="<?= $med_bought[0]['times']; ?>" />
                        ...
                        <!------------------------ first row --------------------------->
                           ...
                           <div class="row">
                            ...
                            <div class="col-md-4 col-sm-4 col-xs-6">
                                <div class="form-group form-floating-label">
                                    ...
                                    <option value="<?=$med_bought[0]['customer_account_id']?>"><?=$med_bought[0]['customer_account_name']?></option>
                            ...
                            <div class="col-md-4 col-sm-4 col-xs-6">
                                ...
                                <div class="input-group mb-3" style="margin-top:10px" data-provide="datepicker">&nbsp;&nbsp;
                                    ...
                                    <span class="input-group-text" style="width:40px !important;" data-mddatetimepicker="true" data-trigger="click"
                                        ...
                                        data-targetselector="#exampleInput00" value="<?=$med_bought[0]['idate']?>" required
                          ...
                          <hr />
                            ...
                            <div class="row">
                              <input type="hidden" id="counter" name="counter" value="<?=count($details)?>"   class="form-control">
                              <?php  $this->load->view('buy/buying/general/editForm'); ?>
                            </div>
                            </div>
