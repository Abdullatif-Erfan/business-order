    ...
    <div class="row">
        <div class="col-md-3 col-sm-4 col-xs-6">
            ...
            value={{ $warehouseItems->first()->id ?? 0 }} >
        ...
        <div class="col-md-3 col-sm-4 col-xs-6">
            ...
            <select class="form-control select2" style="width: 100%; background-color:#ddd;" name="distination_warehouse_id" required>
                ...
                @foreach($warehouses as $warehouse)
                   @if($warehouse->id != $warehouseItems->first()->warehouse_id)
        ...
        <div class="col-md-3 col-sm-4 col-xs-6">
            ...
            <input  name="old_amount" id="old_amount" type="hidden" value={{ $warehouseItems->first()->available_amount ?? 0}} >
            <input class="form-control" name="amount" id="amount" type="number" step="0.01" oninput="checkAmountChanges(this.value)"
                    required  value={{ $warehouseItems->first()->available_amount ?? ''}}  >
        </div>

        <div class="col-md-3 col-sm-4 col-xs-6">
            <label for="amount">واحد  </label>
            <input type="text" class="form-control" readonly value="{{ $warehouseItems->first()?->unitRelation?->first()?->name ?? '' }}">
