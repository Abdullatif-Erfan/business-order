    ...
    <form id="updateProfitForm">
        ...
        <div class="table-responsive">
            <table class="table table-bordered">
                <thead>
                    <tr>
                        <td colspan="9">{{__('buy.buy_tax_percentage_s')}}: % {{$boughtItemDetails->first()->buy_tax_per}} </td>
                    </tr>
                    <tr style="background: #e9fffe;">
                        <th style="width:5%">#</th>
                        <th style="width:15%">{{__('wh.item_selection')}}</th>
                        <th style="width:5%">{{__('common.amount')}}</th>
                        <th style="width:5%">{{__('common.unit')}}</th>
                        <th style="width:13%">{{__('buy.buy_up')}}</th>
                        <th style="width:13%">{{__('buy.profit')}}</th>
                        <th style="width:13%">{{__('sales.sold_up')}}</th>
                        <th style="width:13%">{{__('buy.sell_up_vat')}}</th>
                        <th style="width:13%">{{__('buy.sell_tax_price')}}</th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <input type="hidden" name="items[{{ $index }}][sell_tax_per]" class="sell-tax-per" value="{{ $item->sell_tax_per ?? $item->buy_tax_per ?? 0 }}">
                            <input type="hidden" name="items[{{ $index }}][amount]" class="item-amount" value="{{ $item->amount }}">
                        </td>
                        ...
                        <td>
                            <input type="number" step="any" min="0" 
                                   class="form-control sell-up-vat-display" 
                                   name="items[{{ $index }}][sell_up_vat]" 
                                   value="{{ $item->sell_up_vat ?? '' }}" 
                                   placeholder="{{__('sales.sell_up')}}" readonly>
                        </td>
                        <td>
                            <input type="number" step="any" min="0" 
                                   class="form-control sell-tax-price-display" 
                                   name="items[{{ $index }}][sell_tax_price]" 
                                   value="{{ $item->sell_tax_price ?? '' }}" 
                                   placeholder="{{__('buy.sell_tax_price')}}" readonly>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
...
$(document).ready(function() {
    // =========================================
    // RECALCULATE SELL UP AND TAX ON PROFIT CHANGE
    // =========================================
    $(document).on('input', '.profit-input', function() {
        ...
        var taxPercent = parseFloat(row.find('.sell-tax-per').val()) || 0;
        
        // Calculate sell up
        var sellUp = buyUp + profit;
        row.find('.sell-up-display').val(sellUp.toFixed(2));
        
        // Calculate tax if tax percent > 0
        var sellTaxPrice = 0;
        var sellUpVat = 0;
        
        if (taxPercent > 0 && amount > 0 && sellUp > 0) {
            // Calculate total without tax
            var totalWithoutTax = amount * sellUp;
            
            // Calculate tax amount (VAT)
            sellTaxPrice = (totalWithoutTax * taxPercent) / 100;
            
            // Calculate unit price with VAT
            sellUpVat = sellUp + sellTaxPrice;
        }
        
        // Update the display fields
        row.find('.sell-tax-price-display').val(sellTaxPrice.toFixed(2));
        row.find('.sell-up-vat-display').val(sellUpVat.toFixed(2));
    });

    // =========================================
    // TRIGGER CALCULATION ON PAGE LOAD
    // =========================================
    $('.profit-input').each(function() {
        $(this).trigger('input');
    });
