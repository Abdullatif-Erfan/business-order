...
$(document).ready(function () {
    ...
    // DATA STORAGE
    // =========================================
    var draftOrdersData = {!! json_encode($filteredDraftOrders ?? []) !!};
    ...
    // =========================================
    // GENERATE ROW HTML (For items from orders)
    // =========================================
    ...
    // =========================================
    // GENERATE NEW ROW HTML (With dropdown to select from warehouse)
    // =========================================
    ...
    // =========================================
    // APPEND ROW (For items from orders)
    // =========================================
    ...
    function loadItemsForCustomer(customerId) 
    {
        if (!customerId) {
            currentItems = [];
            $('#itemsBody').empty();
        ...
        // Find draft orders for this customer
        var customerOrders = draftOrdersData.filter(function(order) {
        ...
        customerOrders.forEach(function(order) {
            // Find warehouse item for this pre_list
            var warehouseItem = warehouseItemsData.find(function(wi) {
                return wi.pre_list_id == order.pre_list_id;
            });

            if (warehouseItem && warehouseItem.available_amount > 0) {
                allItems.push({
                    order_id: order.id,
                    dord_num: order.dord_num,
                    pre_list_id: order.pre_list_id,
                    item_name: warehouseItem.item_name || order.preList_relation?.name || 'Unknown',
                    // ✅ Use unit_id and unit_name from the order, NOT from warehouse
                    unit_id: order.unit_id,
                    unit_name: order.unit_relation?.name || 'Unknown',
                    amount: parseFloat(order.amount) || 0,
                    available_amount: warehouseItem.available_amount || 0,
                    warehouse_item_id: warehouseItem.id,
                    sell_up: parseFloat(warehouseItem.sell_up) || 0,
                    total: 0,
                    from_order: true,
                    is_new: false
                });
    ...
    // =========================================
    // AMOUNT VALIDATION - Prevent exceeding availability
    // =========================================
