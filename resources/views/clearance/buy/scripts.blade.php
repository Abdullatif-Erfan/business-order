...
function submitBuyClearanceForm() {
    var currency_id = parseInt($('#buy_currency_id').val()) || 0;
    var buy_to_account_id = parseInt($('#buy_to_account_id').val()) || 0;

    if (currency_id > 0 && buy_to_account_id > 0) {
        // Use template literals to correctly form the route
        var url = "{{ route('clearance.buy.create', ['currency_id' => '__CURRENCY_ID__', 'buy_to_account_id' => '__BUY_TO_ACCOUNT_ID__']) }}"
            .replace('__CURRENCY_ID__', currency_id)
            .replace('__BUY_TO_ACCOUNT_ID__', buy_to_account_id);

        window.location.href = url;
    } else {
        alert('لطفا فروشنده و واحد پولی را انتخاب نمایید');
    }
}
